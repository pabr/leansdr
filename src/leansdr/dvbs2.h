  ...
  struct s2_frame_receiver : runnable {
    ...
    void run() {
      ...
      int min_samples = (1 + MAX_SYMBOLS_PER_FRAME +
			 plslot<SOFTSYMB>::LENGTH)*omega * 2;
	...
	switch ( state ) {
	case COARSE_FREQ:  run_frame_coarse(); break;
	case FRAME_SEARCH: run_frame_search(); break;
...
#define xfprintf(...) {}
...
#if TEST_DIVERSITY
	if ( psymbols ) *psymbols++ = p * scale_symbols;
#endif
	pls_symbols[s] = p;
    ...
    void init_agc(const complex<T> *buf, int n) {
      in_power = 0;
      for ( int i=0; i<n; ++i ) in_power += cnorm2(buf[i]);
      in_power /= n;
    }
  ...
  public:
	...
	if ( nsyncs == MAXSYNCS ) fail("Bug: too many syncs");
	sync *s = &syncs[nsyncs++];
	...
	for ( int i=c->nsymbols; i--; ) {
	  ...
	  else { bit=random_decision; random_decision^=1; }  // Near 0
